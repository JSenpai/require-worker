...
const requireWorkerClient = exports.requireWorkerClient = class requireWorkerClient {
	...
	_construct(file, options = { ownProcess:false, shareProcess:null, parentModule:null, returnClient:false, returnClientPromise:false, forkOptions: null, preConfigureProxy:null }) {
		...
		if(self.rwProcess.worker){
			self.child.once('close',(...args)=>events.emit('workerClose',...args));
			self.rwProcess.worker.once('disconnect',(...args)=>events.emit('workerDisconnect',...args));
			self.rwProcess.worker.once('error',(...args)=>events.emit('workerError',...args));
			self.rwProcess.worker.once('exit',(...args)=>events.emit('workerExit',...args));
		}
		else {
			self.child.once('close',(...args)=>events.emit('workerClose',...args));
		...
		self.proxy = self.proxyCom.createProxy(options.preConfigureProxy?{ preConfigure:options.preConfigureProxy }:{});
			...
			if (this.rwProcess) {
				...
				for (var i = 0, l = removedObjects.length; i < l; i++) {
					...
					if (hasChilds.indexOf(obj.child) === -1) {
						...
						if(obj.worker && obj.worker.isConnected()){
							obj.worker.disconnect();
						}
						else {
							obj.child.unref();
							obj.child.kill();

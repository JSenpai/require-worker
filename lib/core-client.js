...
const requireWorkerClient = exports.requireWorkerClient = class requireWorkerClient {
	...
	_construct(file, options = { ownProcess:false, shareProcess:null, parentModule:null, returnClient:false, returnClientPromise:false, forkOptions: null }) {
		...
		self.child.once('close',(...args)=>this.events && this.events.emit('workerClose',...args));
		self.child.once('disconnect',(...args)=>this.events && this.events.emit('workerDisconnect',...args));
		self.child.once('error',(...args)=>this.events && this.events.emit('workerError',...args));
		self.child.once('exit',(...args)=>this.events && this.events.emit('workerExit',...args));
	...
	restart(options) {
		...
		options.returnClientPromise = true;
		options.returnClient = false;

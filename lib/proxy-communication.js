...
const proxyCom = exports.proxyCom = class proxyCom {
	...
	_preDestroy() {
		...
		for (var [key1, interfaceObj] of this.proxyMap) {
			if (interfaceObj.promiseMap) {
				for (var [key2, promiseObj] of interfaceObj.promiseMap) {
	...
	connectTransportClient(onReady) {
	...
	createMainProxyInterface(options = {}){
		return this.createProxyInterface(_.extend(_.deepExtend({}, options), {basePromise: this.connectTransportClientPromise}));
	}
	
	connectTransportHost(onReady) {
	...
	createProxyInterface(interfaceOptions = { id:0 }){
		var iObj = {options: interfaceOptions, timestamp: Date.now()};
		var proxyHandler = Object.create(proxyHandlerDefaults);
		iObj.promiseMap = new Map();
		proxyHandler.get = (...getArgs)=>this.proxyInterfaceGet({getArgs, interfaceObj: iObj, timestamp: Date.now()});
		var self = this;
		...
		Object.assign(iObj.proxyTarget, proxyTargetDefaults);
		...
		this.proxyMap.set(iObj.proxy, iObj);
		return iObj.proxy;
	}
	
	proxyInterfaceGet(getObj) {
		var {getArgs: [target, property], funcArgs = [], newOperator = false, interfaceObj} = getObj;
		var self = this;
		if (property === 'constructor') {
			getObj.newOperator = true;
			var boundConstructor = this.proxyInterfaceGet.bind(this, getObj);
			boundConstructor.client = this.client;
			boundConstructor.preConfiguredProxy = this.client.requireWorker.preConfiguredProxy.bind(this.client.requireWorker, {interfaceObj, client: this.client});
		...
		var resultFunc = function requireWorkerProxyInvoker(...args) {
			...
			if ('preConfigure' in interfaceObj.options && _.isObject(interfaceObj.options.preConfigure))
				_.deepExtend(promiseObj.configObj, interfaceObj.options.preConfigure);
			promiseObj.promise = new Promise((resolve2, reject2)=>{
				var resolve = (...rArgs)=>{
					self.proxyInterfaceGetEndPromise(promiseObj, {resolve: true});
					resolve2(...rArgs);
				};
				var reject = (...rArgs)=>{
					self.proxyInterfaceGetEndPromise(promiseObj, {reject: true});
					reject2(...rArgs);
				};
	...
	proxyInterfaceGetEndPromise(promiseObj, options = {}){
		...
		if ((options.resolve || options.reject) && interfaceObj.promiseMap.has(promiseObj.promise))
			interfaceObj.promiseMap.delete(promiseObj.promise);
	}
	
	proxyInterfaceGetLateConfigureError() {
		var err = Error(".configure must be called straight after promise creation");
		err.code = "LATE_CONFIGURE";
		throw err;
	}
	
	proxyInterfaceGetPromiseAction(promiseObj) {
		...
		if (_.has(configObj, 'objectPath') && _.isString(configObj.objectPath) && configObj.objectPath.length > 0) actionConfig.objectPath = configObj.objectPath;
		...
		if (_.has(configObj, 'promiseResult') && configObj.promiseResult) actionConfig.promiseResult = true;
		...
		//if(_.has(configObj,'promisify') && configObj.promisify) actionConfig.promisify = !!configObj.promisify;
		if (property === null) {

...
const proxyConstructor = exports.proxyConstructor = class proxyConstructor {
	...
	proxyTargetConstructor(constructorOptions) {
		...
		var self = this;
		if (property === 'constructor') {
			constructorOptions.newOperator = !!newOperator;
	...
	handlePromiseArguments(proxyPromise,actionData){
		...
		var callbackOnRemove = _.has(proxyOptions,'callbackOnRemove') && _.isFunction(proxyOptions.callbackOnRemove) && ((...a)=>{
			try{ proxyOptions.callbackOnRemove(...a); }catch(err){};
		});
		args.forEach((arg,i)=>{
			if(_.isFunction(arg)){
				var returnEventName = 'proxyActionArgument:' + interfaceID + ':' + actionID + ':' + i;
				args[i] = null;
				...
				if(callbackLimit>1) {
					...
					callArg(...funcArgs);
					removeCancelListener();
					if(callbackOnRemove) callbackOnRemove(i);
				};

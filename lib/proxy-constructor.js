...
const proxyConstructor = exports.proxyConstructor = class proxyConstructor {
			...
			if (proxyInterface.promiseMap) {
				for (var [key2, promiseObj] of proxyInterface.promiseMap) {
					if (promiseObj.reject && !promiseObj.resolved && !promiseObj.rejected) {
						try {
							promiseObj.reject(this.proxyCom.proxyHandler.proxyInterfaceHandleResultError({
								code: 'DESTROYED',
	...
	endProxyPromise(proxyPromise, options = {}){
		...
		if (options.timeout) {
			...
			var rejectError = this.proxyCom.proxyHandler.proxyInterfaceHandleResultError({
				code: 'TIMEOUT',
	...
	proxyPromiseAct(proxyPromise) {
		...
		this.proxyCom.transport.once(returnEventName, (...args)=>this.proxyCom.proxyHandler.proxyInterfaceHandleResult(proxyPromise, actionData, ...args));

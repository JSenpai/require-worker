...
var ipcTransport = function(options={}){
...
ipcTransport.prototype = {
	...
	setChild: function(child){
		...
		child.on('message',([id,...data])=>{
			if(id===this.id){
				this.incomingChildMessage(...data);
			}
		});
		child.on('disconnect',()=>this.disconnectEvent());
	},
	incomingChildMessage: function(...data){
	...
	setParent: function(){
		...
		process.on('message',([id,...data])=>{
			if(id===this.id){
				this.incomingParentMessage(...data);
			}
		});
		process.on('disconnect',()=>this.disconnectEvent());
	},
	incomingParentMessage: function(...data){
		this.events.emit('message',...data);
	},

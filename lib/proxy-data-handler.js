...
dataHandler.prototype = {
	...
	proxyTargetAction: function(actionData){
		...
		.then((resultObj)=>{
			if('resultType' in resultObj) this.proxyTargetActionReply(actionData,resultObj);
	...
	actionHandleOperation: function(handleData){
		...
		if('deleteProperty' in actionConfig && actionConfig.deleteProperty){
			try{
				...
				return Promise.resolve({ resultType:'value', result:true });
			}catch(err){
		...
		// Has Own Property
			...
			return Promise.resolve({ resultType:'value', result:result });
		}
		// Has Property
		else if(_.has(actionConfig,'hasProperty') && actionConfig.hasProperty){
			return Promise.resolve({ resultType:'value', result:targetPropertyExists });
		}
		// Set Property
	...
	actionHandleDataType: function(handleData){
		...
		// Basic data types
		if(resultTarget===null || resultTarget===true || resultTarget===false || resultTarget===void 0 || _.isString(resultTarget) || _.isNumber(resultTarget)){
			return Promise.resolve({ resultType:'value', result:resultTarget });
		}
		// todo: implement: date, regex, NaN? (have resultType as 'valueDate', 'valueRegex', etc)

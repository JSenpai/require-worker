...
dataHandler.prototype = {
	_destroy: function(){
		...
		this.proxyTargetAction = this.proxyTargetActionReply = ()=>{ throw Error("dataHandler has been destroyed"); };
	},
	proxyInterfaceHandleResult: function(promiseObj,actionData){
		var { resolve, reject } = promiseObj;
		var { error, resultType, result } = actionData;
		if(error) return reject(this.proxyInterfaceHandleResultError(promiseObj,error));
	...
	proxyTargetActionReply: function(actionData,resultObj){
		if(actionData.sendMainReply) throw Error("proxyTargetActionReply already sent main reply, actionData:",actionData);
		actionData.sendMainReply = true;

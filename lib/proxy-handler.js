...
const dataHandler = exports.dataHandler = class dataHandler {
		...
		if (this.proxyTargetActionMap) {
			for (var [key, handleData] of this.proxyTargetActionMap) {
				...
				if (!actionData.sentMainReply) {
					try {
						this.sendActionResponse(actionData, {
	...
	onProxyAction(actionData) {
		...
		.then(()=>this.handleActionOperations(handleData))
		.then((resultObj)=>{
	...
	handleActionOperations(handleData) {
		...
		// Set Property Via New Operator (if property on target does not exist)
			...
			return this.handleActionOperations(handleData);
		}
		...
		// Get
			...
			return this.handleActionData(handleData);
		}
	}
	
	handleActionData(handleData) {

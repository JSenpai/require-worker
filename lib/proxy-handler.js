...
const dataHandler = exports.dataHandler = class dataHandler {
	...
	_proxyInterfaceHandleResultParseObjPromiseGet() {
	...
	_proxyInterfaceHandleResultParseObj(actionData, obj, next) {
		...
		Object.defineProperty(obj, 'promise', {get: this._proxyInterfaceHandleResultParseObjPromiseGet});
	...
	proxyInterfaceHandleResult(promiseObj, actionData, resultObj) {
		...
		var resolve = (obj)=>this._proxyInterfaceHandleResultParseObj(actionData, obj, resolveOriginal);
		var reject = (obj)=>this._proxyInterfaceHandleResultParseObj(actionData, obj, rejectOriginal);
		if (error) {
		...
		if (type === 'value') return resolve({value: result});
			...
			if(actionData.actionConfig.followPromise) return result ? resolve({ value: resultObj.value }) : reject(this.proxyInterfaceHandleResultError({ value: resultObj.value }));
	...
	proxyInterfaceHandleResultError(error) {
	...
	proxyTargetActionReply(actionData, resultObj) {
	...
	actionResolveTarget(handleData) {
	...
	actionHandleOperation(handleData) {
	...
	actionHandleDataType(handleData) {
		...
		// Handle get object objectPath option
			...
			return Promise.reject({error: {code: 'NOT_YET_IMPLEMENTED', message: "objectPath feature not yet implemented"}});
		}
